...
(module ROOT/CPhCWc9MMHQV/CPTAGGehn7eP/CPVHDqKYNUwz/CPYWYcV8dwCE/CPQU4nPmdVqN racket 
  (require rackunit 'CODEPOD "../../../../../../ROOT/CPhCWc9MMHQV/CPTAGGehn7eP/CPVHDqKYNUwz/CPYWYcV8dwCE/CPckUrdGFJME/main.rkt" "../../../../../../ROOT/CPhCWc9MMHQV/CPTAGGehn7eP/CPVHDqKYNUwz/CP9hPyBA9z8P/main.rkt")
...
(define (*--proc lst)
  (let ([item-1 (first lst)]
       ...
       (hook-proc! 
         res (Net (list (pin-ref res 'left)
                    (pin-ref item-1 'left))))
       ;; end
       (hook-proc! 
         res (Net (list (pin-ref item-n 'right)
                    (pin-ref res 'right))))
       (for/fold ([prev (first lst)])
         ([cur (rest lst)])
         (hook-proc!
           res (Net (list (pin-ref prev 'right)
                      (pin-ref cur 'left))))
         cur)
       res))
